    ...
    <div class="col-12 grid-margin stretch-card">
        <div class="card">
          <div class="card-body">
            ...
            <form method="POST" action="{{route('product.uploads.set.update', $product->id)}}" class="forms-sample" enctype="multipart/form-data">
              @csrf
              <div class="form-group select1">
                <label for="">Chọn Ảnh Bìa Hiển Thị</label>
                <select class="js-select1" name="default">
                  @foreach($product->images as $images)
                  <option value="{{$images->product_image_path}}"
                  @if($default->product_image_path == $images->product_image_path) selected @endif>Hình {{$loop->iteration}}</option>
                  @endforeach
                </select>
              </div>
                </br>
              ...
              <div class="form-group select2">
                ...
                <select class="js-select2" name="hover">
                  @foreach($product->images as $images)
                  <option value="{{$images->product_image_path}}"
                  @if($hover->product_image_path == $images->product_image_path) selected @endif>Hình {{$loop->iteration}}</option>
                  @endforeach
                </select>
              </div>
